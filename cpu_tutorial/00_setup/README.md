...
- `diff` reports a missing file → the verification command must run from the repository root (the golden path starts with `cpu_tutorial/...`), while the paths inside `main.cpp` are relative to this module folder.
