// 09_forward -- full single-step forward pass, FP32 (reference solution)
//
// Assembles the ONE-layer block of module 08_transformer_layer into the
// complete forward(token, pos) -> logits:
//   x = embedding[token]
//   for l in 0..n_layers-1: the transformer layer of module 08
...
// s.x at position pos (module 08_transformer_layer, parameterized by l)
// ---------------------------------------------------------------------------
...
std::span<const float> forward(int token, int pos, const tut::Config& p,
    ...
    // embedding lookup: copy this token's row of the table into s.x
    std::ranges::copy(w.token_embedding_table.subspan(static_cast<size_t>(token) * dim, dim),
                      s.x.begin());

    // stack the layers: the same block 6 times, each with its own weights
    // and its own KV cache slice
    for (int64_t l = 0; l < p.n_layers; l++) { transformer_layer(l, pos, p, w, s); }
