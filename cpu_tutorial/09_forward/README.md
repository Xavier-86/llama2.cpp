...
Fill in the 3 TODOs inside `forward()` in `main.cpp`, stacking the single transformer layer of module 08_transformer_layer 6 times and adding the output head, completing `forward(token, pos) -> logits`:

```
x = embedding[token]                                  # table lookup (given)
for l in 0..n_layers-1:                               # 6 layers (subtask 1)
...
x = rmsnorm(x, rms_final_weight)                      # (subtask 2)
logits = Wcls @ x                                     # (subtask 3) shared embedding table
```

This is the FP32 grand assembly and the first milestone acceptance — once it passes, you have rebuilt `run.cpp`'s `Transformer::forward`. This module is the second half of what used to be a single "forward" module; the first half (one layer) is module 08_transformer_layer.
...
Because the single layer is already verified, this module gives you all of it: the six kernels `rmsnorm`, `softmax`, `matmul`, `rope`, `attention`, `ffn` (modules 03-07), the single-layer assembly `transformer_layer(l, pos, p, w, s)` (module 08), and the embedding lookup are **given code** in main.cpp, as is all of `main()`. The new work is only the stacking loop and the output head.
...
**Outputs**: `main()` is already written — it calls `forward` for each of the 5 prompt tokens (pos = 0..4), collects every position's logits, and takes their argmax. Output files (written into this module folder):
...
## Subtask 1: stack the layers

Call `transformer_layer(l, pos, p, w, s)` for every `l` in `0 .. p.n_layers-1`, in order. The given function already handles all per-layer offsets; your job is only the loop.

Background you need — what the given `transformer_layer` does. You assembled it in module 08_transformer_layer; here it is parameterized by the layer index l, slicing weights at `l * per-layer size` and the cache at `l * seq_len * kv_dim`. Two conventions from that module matter here:
...
Background you need — `RunState`, the activation and cache buffers the layers operate on. It is allocated once from the config and reused across all positions (given in main.cpp — do not modify):
...
## Subtask 2: final RMSNorm

After the last layer, apply rmsnorm in place on `s.x`, with `w.rms_final_weight`.

Background you need — the head's norm weight is its own: `rms_final_weight` is a single (288,) weight, not a per-layer slice like `rms_att_weight` / `rms_ffn_weight`.

## Subtask 3: classifier head

One more matmul produces the logits: `matmul(s.logits, s.x, w.wcls)`.

Background you need — weight sharing: for this model `wcls` **is** the embedding table (the same span), so the classifier head reuses `w.token_embedding_table`.
