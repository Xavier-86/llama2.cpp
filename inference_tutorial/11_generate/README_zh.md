...
> 目标：将 forward（09）、sampler（10）和 tokenizer（02）串成完整的文本生成流程。本模块几乎不引入新数学——通过它意味着你已重建 FP32 `run.cpp`。

## 背景

前面每个模块都是"一次调用"：一次前向、一次采样。真正的文本生成是一个**循环**：模型每步只预测下一个 token，把这个 token 喂回去，再预测下一个。这个循环有两个阶段，对应服务系统里的两个核心指标（TTFT ≈ prefill 时间，吞吐量 ≈ decode 速度）。

本模块的代码骨架已经搭好：checkpoint 解析（模块 01 的产物）由 `../common/checkpoint.h` 的 `tut::load_checkpoint` 提供，词表加载由 `../common/tokenizer.h` 的 `tut::load_vocab` 提供，输出写文件由 `../common/io.h` 提供。你在模块 02–10 中实现的组件——数学内核、完整前向、BPE 分词器、采样器——在 `main.cpp` 中已作为给定代码提供。你要写的只有唯一的新部分：生成循环。

## 工作原理

### prefill 与 decode 两阶段

设提示词编码后得到 `prompt_tokens`（长度 P=5：`[1, 9038, 2501, 263, 931]`，其中 1 是 BOS `<s>`）。循环每一步做一次 `forward(token, pos)`，然后决定下一步的输入 `next`：
...
`forward(token, pos)` 每步只为**当前这一个 token** 计算 K/V，写入 `key_cache[layer][pos]` / `value_cache[layer][pos]`；注意力读取的是第 `0..pos` 行——历史位置的 K/V 全部来自缓存，不重算。prefill 阶段就是在为 decode 阶段"预热"这个缓存：没有 prefill 填进去的前 P 行，decode 第一步的注意力就没有上下文可看。这也是为什么 decode 每步的计算量与序列位置无关（一个 token 的前向），只有注意力的历史长度随 `pos` 线性增长。

### 两组运行参数

`main()` 固定跑两组（提示词均为 `"Once upon a time"`，`steps = 64`）：
...
采样运行的可复现性完全依赖 RNG：每步恰好消耗一个随机数（xorshift64，见模块 10），顺序必须与参考实现一致——多调用一次，从第一步起全部分歧。

## 输入数据

不需要自己解析任何文件；输入以下来源都已就绪：
...
| temperature / topp / seed | main() 内联 | 见上表 | 两组采样参数 | — |
...
前序组件在 `main.cpp` 中均为**给定代码**——数学内核（模块 03/04）、完整前向（模块 09）、BPE 分词器（模块 02）、采样器（模块 10）已按参考实现填好。本模块唯一的新工作是 `TODO(task 6)`：

1. **task 6 — `generate()`**：按上面的伪代码实现 prefill/decode 循环，产出 `ids`（循环产生的每个 `next`，含 prefill 阶段强制输入的 4 个，不含 BOS 本身）与 `text`（`decode(token, next)` 片段的拼接）。
...
`main()` 已写好：加载 checkpoint 与词表，跑贪心与采样两组，写出 4 个文件。文本文件写出时追加一个尾部换行（`g.text + '\n'`），与 golden 数据的约定一致。
