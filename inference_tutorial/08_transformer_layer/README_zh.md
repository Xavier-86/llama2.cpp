...
**输入**：提示词输入是 main.cpp 里的 const 数组，模型权重由 `tut::load_checkpoint("../../stories15M.bin")` 加载，返回 `tut::Checkpoint{config, weights, buffer}`，11 个权重张量都是指向 buffer 的 `std::span`，零拷贝。本模块只用 embedding 表和每个按层张量的**第 0 层切片**（均为层主序，第 0 层就是每个张量偏移 0 处的第一块）：
