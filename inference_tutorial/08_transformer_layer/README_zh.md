...
补全 `main.cpp` 中的 TODO，把模块 03～07 的内核组装成**一个** transformer 层（只用第 0 层权重）——也就是构成整个模型的重复积木。一次 `forward_layer0` 调用处理**一个** token 在**一个**位置 pos 上的 embedding 查表和第 0 层计算；P 个 token 的提示词需要按顺序调用 P 次（prefill 阶段），每个位置都会用到 KV cache 中前面所有位置的 k/v，这正是 cache 存在的意义：

```
"Once upon a time" -> token {1, 9038, 2501, 263, 931} -> forward_layer0 × 5（pos 0..4）
   每个位置：embedding 查表 -> rmsnorm -> qkv -> 写 cache + rope -> attention
             -> wo + 残差 -> rmsnorm -> ffn + 残差
```

本模块是原"forward"模块拆分的前半——后半（完整 6 层 + 最终归一化 + 分类头）在模块 09_forward。拆成两半是为了缩小调试范围：如果本模块的单层激活能与黄金数据对上，模块 09 就只是重复（6 层）加上一个头（最终 rmsnorm + 分类头）。

checkpoint 解析已由 `../common/checkpoint.h` 的 `tut::load_checkpoint` 完成（它就是模块 01 的打包答案），输出文件的写出用 `../common/io.h`。本模块代码只剩算法本身。

**输入**：提示词输入是 main.cpp 里的 const 数组，模型权重由 `tut::load_checkpoint("../../models/stories15M.bin")` 加载，返回 `tut::Checkpoint{config, weights, buffer}`，11 个权重张量都是指向 buffer 的 `std::span`，零拷贝。本模块只用 embedding 表和每个按层张量的**第 0 层切片**（均为层主序，第 0 层就是每个张量偏移 0 处的第一块）：
...
**输出**：`main()` 已写好——对 5 个提示词 token 依次调用 `forward_layer0`（pos = 0..4），收集每个位置的两份记录并写出两个输出文件（写在模块目录下），均为**位置优先拼接**（pos 0 的 288 个值、接着 pos 1 的……各 5×288 行）：
...
`data/expected_*` 是黄金数据，不要修改。设置两个检查点而不是一个，是为了出错时能区分注意力半边和 FFN 半边：如果 `out_att_residual.txt` 已经对不上，bug 在步骤 b)~f)；如果只有 `out_layer_out.txt` 对不上，查步骤 g)~h)。

## 子任务一：复制你的内核 `TODO(module 03..07)`

把模块 03～07 写好的内核原样复制进各自的 stub（各标注 `TODO(module 0N)`）：`rmsnorm`、`softmax`、`matmul`、`rope`、`attention`、`ffn`——这些不是新工作，签名校验照搬即可。

## 子任务二：task 1——embedding 查表

`forward_layer0()` 中的 `TODO(task 1)`：把 `w.token_embedding_table` 的第 `token` 行（vocab_size × dim，行主序）复制到 `s.x`。第 `token` 行在表内偏移 `token * dim` 处——见上面的输入表。

## 子任务三：task 2——一个 transformer 层

`forward_layer0()` 中的 `TODO(task 2)`：按下面数据流的 a)~h) 顺序执行。只跑第 0 层，所以所有权重切片都从各自张量的偏移 0 开始（`wq`：`dim*dim` 个浮点；`wk`/`wv`：`dim*kv_dim`；`w1`/`w2`/`w3`：`dim*hidden`；rmsnorm 权重：`dim`），cache 切片就是整个 `key_cache`/`value_cache`，位置 pos 的行在其内偏移 `pos * kv_dim`。k/v 直接写进 cache 行，**先写 cache 再 rope**。两次残差都是 `x[i] += ...`。
...
需要的知识——`RunState`，激活与缓存缓冲区（main.cpp 中已给出，无需修改）。它由 config 一次性分配，跨所有位置复用。相比模块 09 做了裁剪：没有 `logits`（本模块没有分类头），cache 只存一层：
