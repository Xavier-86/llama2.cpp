...
补全 `main.cpp` 中的 5 个 TODO，实现 Transformer 的核心——带 KV cache 的多头因果注意力。这是最容易出现隐蔽错误的模块，请预留更多时间。

每一层的前向传播中，注意力让当前位置"回看"它之前的所有位置：用 query 和历史上每个位置的 key 算相似度，归一化成权重后对 value 做加权和：

```
q、K/V cache 第 0..pos 行  -->  score_t = (q · k_t) / sqrt(head_size)  -->  softmax  -->  权重  -->  对 v_t 加权和  -->  out
```

生成是自回归的——位置 pos 只能看到 0..pos（**因果掩码**），所以推理时把每步算好的 K/V 存进 **KV cache**，后续位置直接复用，不必重算整个前缀。
...
**输入**：所有输入都是 data.h 里的 const 数组（由 `../tools/embed_data.py` 从 `data/*.txt` 生成，值完全一致，勿手改 data.h），无需读任何文件：
...
**输出**：`main()` 的其余部分已写好——对 pos = 0..4 逐位置调用 `attention`，输出 out.txt（5×288，所有位置拼接）；task 5 由你写出 out_att.txt（最后一个位置的注意力权重）。`data/expected_*` 是黄金数据，不要修改。

## 子任务一：head 循环与 query 切片（task 1）

遍历 head h = 0..5，求 `kv_h = h / kv_mul`，从 q 中切出本 head 的 `qh`（`h * head_size` 起的 48 个值）。

需要的知识——q 的 head 切分。dim=288 的 q 切成 `n_heads` 个 head，每个长 `head_size`，head h 占：

```
qh = q[h*head_size : (h+1)*head_size]
```

需要的知识——GQA：query head h 对应的 KV head 是 `kv_h = h / kv_mul`，`kv_mul = n_heads / n_kv_heads`。stories15M 中 n_heads = n_kv_heads = 6，故 kv_mul = 1、kv_h = h（每个 head 各有自己的 K/V）；GQA 模型（n_kv_heads < n_heads）下多个 query head 会共享同一组 K/V。请按通用 kv_mul 写代码，不要硬编码 kv_h = h。

## 子任务二：0..pos 上的注意力分数（task 2）

对 t = 0..pos（仅历史+当前），从 k_cache 第 t 行切出 kv_h 的 key（起点 `t * kv_dim + kv_h * head_size`），算点积 `qh · key` 并除以 `sqrt(head_size)`，存入本 head 的 att 切片下标 t。
...
需要的知识——KV cache 的布局。真实推理中 cache 形状为 (n_layers, seq_len, kv_dim)：每个位置生成时，把该层新算出的 k、v 写入第 pos 行；之后所有位置都能读到 0..pos 行。本模块直接给你第 0 层 cache 的前 P=5 行切片，pos-major 摊平成一维：
...
## 子任务三：对 0..pos 段做 softmax（task 3）

只对本 head att 切片的 0..pos 段做 softmax——复用给定的 `softmax`，它在共享缓冲区上**原地**工作（模块 03 的约定），未来位置的槽位不能参与 exp。

需要的知识——softmax 的范围恰好是你刚写入的 pos+1 个分数：

```
att = softmax(score_0..pos)                # 只对这 pos+1 个 score 做 softmax
```

## 子任务四：对 V 做注意力加权和（task 4）

把本 head 的 out 切片清零，按与 K 相同的行/head 切法从 v_cache 取行，累加 `att_t * v_t`。

需要的知识——加权和公式与 head 拼接：
...
## 子任务五：导出最后位置的注意力权重（task 5，在 `main()` 里）

循环结束后 att 里留的是最后位置 pos=4 的权重；按 head-major 收集（head 0 的 5 个、head 1 的……共 30 个）写入 out_att.txt。
