...
补全 `main.cpp` 中的 TODO（都在 `Sampler` 类和 `softmax` 里）：实现三种采样策略——贪心 argmax、全分布采样、top-p 核采样——以及一个必须逐 bit 复现参考实现的 xorshift 随机数生成器。输入 32000 个分数，输出一个 token id。
...
**输出**：`main()` 已经写好——先做 RNG 自检（seed=42 的前 10 个硬币）写出 `out_rng.txt`，再跑 8 个用例写出 `out.txt`。前 4 个用真实 logits，后 4 个用合成 logits，参数为 (temperature, topp, seed)：
...
真实分布有 96.6% 的概率集中在 argmax（token 29892）上，所以 4 个真实用例都选中它——真正考验 mult / top-p 实现的是合成用例。`data/expected_*` 是黄金数据，不要修改。

## 子任务一：xorshift 随机数 `random_u32()` / `random_f32()`（task 1）

按下面的公式推进 xorshift 状态并取高 32 位（注意是 64 位无符号乘），再把 u32 映射成 `[0, 1)` 的 float。先让 out_rng.txt 对上再继续。

需要的知识——xorshift 算法（必须与参考实现逐 bit 一致）。64 位状态 `st`（种子即初始状态），每次取数：
...
## 子任务二：`softmax(x)`（task 2）

把 `x` 原地变成概率分布，数值稳定（先减最大值再 exp）：减最大值、取 exp、再除以总和归一化。

## 子任务三：分派 `sample(logits)` + 贪心 `sample_argmax`（task 3、3a）

按下表对 `temperature_` / `topp_` 分派；`sample_argmax` 返回最大元素下标。
...
## 子任务四：`sample_mult(probabilities, coin)`（task 4）

需要的知识——全分布采样。从左到右累加概率（CDF 游走），返回第一个使累计和超过 coin 的下标：
...
## 子任务五：`sample_topp(probabilities, topp, coin)`（task 5）
